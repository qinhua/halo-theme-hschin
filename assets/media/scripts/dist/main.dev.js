...
if (sidebarToggle) for (var i = 0; i < sidebarToggle.length; i++) {
  sidebarToggle[i].addEventListener('click', function (e) {
    var headCtrl = $('#headerCtrl');
    var menuCtrl = $('#sidebarToggle');
    if (headCtrl) headCtrl.toggleClass('no-show');
    if (menuCtrl) menuCtrl.toggleClass('menu-ctrl-on');
    var scrollMenu = $('#scrollMenu');
    if (scrollMenu) scrollMenu.toggleClass('scroll-menu-show');
    var siteHeader = $('#siteHeader');
    if (siteHeader) siteHeader.toggleClass('site-header-scroll');
    siteHeader.toggleClass('site-header-bg');
    var sideBar = $('#sidebar');
    if (sideBar) sideBar.toggleClass('sidebar-show');
    e.preventDefault(); // 滚动条
...
var han = {
  ...
  refreshLazyLoad: function refreshLazyLoad() {
    $('#page').addClass('transform-none');
  },
  lazyLoadCardItem: function lazyLoadCardItem() {
    $(".card-item-vel").velocity("transition.slideUpIn", {
      stagger: 200,
      display: "flex"
    });
  },
  linksAddBlank: function linksAddBlank() {
  ...
  scollHeader: function scollHeader() {
  ...
  makeMenuInvisible: function makeMenuInvisible() {
  ...
  // 展开子菜单
  foldSubMenu: function foldSubMenu() {
    ...
    $('body').on('click', '#pagination a', function () {
      ...
      $.ajax({
        ...
        success: function success(data) {
          ...
          pageInner = $(data).find("#pagination .inner"); // In the new content
...
$(function () {
  ...
  han.linksAddBlank(); // 头部菜单滚动时间

  han.scollHeader();
  han.toggleSearch(); // 当前目录菜单高亮

  han.highlightMenu(); // 菜单点击事件
...
$(document).ready(function () {
  ...
  if (treeHoleBoxs) for (var i = 0; i < treeHoleBoxs.length; i++) {
